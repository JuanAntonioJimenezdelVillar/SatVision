...
def get_args():
    ...
    parser.add_argument('--root_path', default='./data/iSAID_patches', help='root dir of iSAID dataset')
    parser.add_argument('--save_file_name', default='gray_masks', help='save dir of converted gray masks of iSAID dataset')
    parser.add_argument('--phase', choices=['train', 'val'], required=True)

    args = parser.parse_args()
    return args

def process_mask(token, masks_path, save_path):
    ...
    print(f'Converted {token} to gray mask, {labels}')

def main():
    args = get_args()
    masks_path = osp.join(args.root_path, args.phase, 'images/masks')
    ...
    with ProcessPoolExecutor() as executor:
        futures = [executor.submit(process_mask, token, masks_path, save_path) for token in mask_names]
